...
class RetNetModel(nn.Module):
    ...
    def forward_embedding(
        ...
        # if using recurrent and not the first step, only take the last token
        if incremental_state is not None and not self.is_first_step(incremental_state):
            tokens = tokens[:, -1:]

        # embedding to embed_dim
    ...
    def forward(self,
        ...
        # 1. embedding
        # 2. generate relative position encoding
        ...
        if self.chunkwise_recurrent and prev_output_tokens.size(1) % self.recurrent_chunk_size != 0:
            ...
            # # of generated tokens so far
            slen = prev_output_tokens.size(1) # slen is used to generate relative position encoding
        # relative position
