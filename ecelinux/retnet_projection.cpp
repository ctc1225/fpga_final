//==========================================================================
// retnet_projection.cpp
//==========================================================================
// @brief: Projection layer only (for testing)
...
void dut(hls::stream<bit32_t> &strm_in, hls::stream<bit32_t> &strm_out) {
  ...
  for (int i = 0; i < D_e * IN_BIT_WIDTH / BUS_WIDTH; i++) {
    ...
    for (int j = 0; j < BUS_WIDTH; j++) {
      input[bitcount / IN_BIT_WIDTH][bitcount % IN_BIT_WIDTH] = input_l(j, j);
      bitcount++;

    }
  }
  // for (int i = 0; i < D_e; i++) {
  //   std::cout << "print out input in projection module: " << input[i] << std::endl;
  // }
  // call retnet_projection_xcel
  retnet_projection_xcel(input, output);
  ...
  for (int i = 0; i < D_e * PROJECTION_OUT_BIT_WIDTH / BUS_WIDTH; i++){
    for (int j = 0; j < BUS_WIDTH; j++) {
      output_l(j, j) = output[bitcount / PROJECTION_OUT_BIT_WIDTH][bitcount % PROJECTION_OUT_BIT_WIDTH];
      bitcount++;
    }
...
//----------------------------------------------------------
// retnet_projection_xcel Accelerator
//----------------------------------------------------------
...
void retnet_projection_xcel(ubit8_t input[D_e], ubit16_t output[D_e]) {
  ...
  projection<D_e, D_e>(input, output, w_WQ);
